"""Command-line entry point for TermDraft."""
...
from termdraft.bindings import format_command_help
...
from termdraft.models.workspace import Workspace, WorkspaceError

SignalHandler = Callable[[int, FrameType | None], object] | int | signal.Handlers | None


def _run_with_shutdown_signals(app: TermDraftApp) -> None:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--config-dir",
        type=Path,
        help="configuration directory (default: ~/.termdraft)",
    )
    parser.add_argument(
        "--safe-mode",
        action="store_true",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"termdraft: configuration error: {error}", file=sys.stderr)
        return 2
    ...
    try:
        ...
        print(f"termdraft: error: {error}", file=sys.stderr)
        return 2

    _run_with_shutdown_signals(
        TermDraftApp(workspace, config=config, use_user_theme=not arguments.safe_mode)
    )
    return 0
