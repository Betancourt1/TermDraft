...
from termdraft.bindings import (
    APP_BINDINGS,
...
from termdraft.config import ConfigError, TermDraftConfig, get_config_root, load_config
...
from termdraft.screens.semantic_reader import SemanticReaderDialog
...
from termdraft.services.semantic_blocks import (
    SemanticBlock,
    SemanticBlockMap,
    map_semantic_blocks,
)
...
from termdraft.widgets.file_tree import ExplorerResizeHandle, FileExplorer
from termdraft.widgets.preview import EMPTY_WORKSPACE_GUIDANCE, MarkdownPreview
from termdraft.widgets.status_bar import TermDraftStatusBar

_MONOCHROME_THEME = Theme(
    name="termdraft-monochrome",
    primary="#303030",
    secondary="#505050",
    warning="#b8b8b8",
    error="#e8e8e8",
    ...
    variables={
        ...
        "markdown-h1-color": "#e6e6e6",
        "markdown-h2-color": "#d0d0d0",
        "markdown-h3-color": "#b8b8b8",
        "markdown-h4-color": "#a0a0a0",
        "markdown-h5-color": "#909090",
        "markdown-h6-color": "#808080",
        "scrollbar": "#5f5f5f",
...
class TermDraftApp(App[None]):
    ...
    def __init__(
        ...
        *,
        ...
        self.config = config or TermDraftConfig(root=get_config_root())
        ...
        if config is not None and use_user_theme and theme_path.exists():
            try:
                ...
                self._theme_warning = (
                    ...
                    "Fix the file and restart TermDraft to re-enable it."
                )
            else:
    ...
    def compose(self) -> ComposeResult:
        yield Static(f"TermDraft  ·  {self.workspace.root}", id="title-bar", markup=False)
    ...
    @property
    def workbench_resize_handle(self) -> WorkbenchResizeHandle:
        return self.query_one(WorkbenchResizeHandle)
    ...
    def _mark_external_warning(
        ...
        *,
        ...
        if change.kind is ExternalChangeKind.DELETED:
            message = f"{document.path.name} was deleted outside TermDraft"
        elif change.kind is ExternalChangeKind.INACCESSIBLE:
            message = f"{document.path.name} cannot be read or verified"
        else:
            message = f"{document.path.name} changed outside TermDraft"
