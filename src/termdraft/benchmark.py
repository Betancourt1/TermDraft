"""Repeatable development benchmarks for TermDraft's real coordination paths."""
...
from termdraft.services.session import SessionStore
...
def _benchmark_app(initial_path: Path, state_root: Path) -> TermDraftApp:
    return TermDraftApp(
...
async def _wait_for_idle(app: TermDraftApp) -> None:
    for _ in range(2000):
        if (
    ...
    raise RuntimeError("TermDraft did not become idle during the benchmark")


async def _run_watcher_pass(app: TermDraftApp) -> None:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        prog="termdraft-benchmark",
        description="Measure TermDraft semantic parsing, mounted tabs, and watcher passes.",
    )
