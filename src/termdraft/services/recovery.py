...
def default_recovery_root() -> Path:
    """Return the preferred platform recovery leaf, with legacy fallback."""
    if state_home := os.environ.get("XDG_STATE_HOME"):
        state_root = Path(state_home).expanduser()
        canonical = state_root / "termdraft" / "recovery"
        legacy = state_root / "termwriter" / "recovery"
