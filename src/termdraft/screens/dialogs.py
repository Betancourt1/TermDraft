"""Typed modal decisions used by the TermDraft coordinator."""
...
from termdraft.models.workspace import EDITABLE_SUFFIXES, Workspace
...
from termdraft.services.recovery import RecoveryRecord
...
class ConflictDialog(ModalScreen[ConflictDecision | None]):
    ...
    def compose(self) -> ComposeResult:
        if self.can_reload:
            message = f"{self.path.name} changed outside TermDraft. Choose which version to keep."
...
class WorkspaceEntryDialog(ModalScreen[bool]):
    ...
    def update_creation_warning(self, event: Input.Changed) -> None:
        ...
        if has_hidden_part or has_file_like_folder or suffixes > 1:
            ...
            warning = (
                "Unusual file extension: the file will be created but will not open in TermDraft."
            )
...
class HelpDialog(ModalScreen[None]):
    ...
    def __init__(self, content: str, *, title: str = "TermDraft shortcuts") -> None:
